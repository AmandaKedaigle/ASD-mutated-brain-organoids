...
choosePower <- function(seur, powers=NULL, genesToUse=NULL, save=T, plot=T)
{
dat=seur@assays$RNA@data

if (is.null(genesToUse)) {
dat<-dat[!(Matrix::rowMeans(dat, na.rm = TRUE) < 0.15),] #get rid of lowly expressed genes
dat<-dat[!(Matrix::rowMeans(dat, na.rm = TRUE) > 9),] # get rid of highly expressed genes
